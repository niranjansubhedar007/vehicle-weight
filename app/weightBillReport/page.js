...
//       const isWithinSelectedDate = weightDate >= new Date(startDate) && weightDate <= new Date(endDate);
//       const isMatchingSelectedDate = weightDate.toDateString() === new Date(startDate).toDateString();

//       return (
//         (weight.RSTNo.toString().includes(searchTerm) || weight.selectedMaterialName.toLowerCase().includes(searchTerm.toLowerCase())) &&
...
//     const dateRange = startDate && endDate ? `${startDateFormatted} - ${endDateFormatted}` : "(All Dates)";
...
// Calculate the total sum of net weight
const calculateTotalNetWeight = () => {
  ...
  return total.toFixed(2); // Ensure the result is a string with 2 decimal places
};
  ...
  return (
    <div>
      <>
        <Sidebar />
        <div className="p-4 lg:pl-72 lg:w-full md:pl-72 md:w-full min-h-screen">
                  ...
                  <tbody>
                    {searchResults.map((weight, index) => (
                      <tr className="text-center text-base" key={weight._id}>
                        ...
                        <td className="border px-4 py-2">{weight.RSTNo}</td>
                        <td className="border px-4 py-2">
                          {new Date(weight.createdAt).toLocaleDateString("en-IN")}
                        </td>
                        <td className="border px-4 py-2">
                          {weight.vehicalOwnerName}
                        </td>
                    ...
                    <tr>
                      <td colSpan="10" style={{ textAlign: 'right', fontWeight: 'bold' }} className="p-2">Total Net Weight:</td>
                      <td style={{ fontWeight: 'bold' }} className="p-2">{calculateTotalNetWeight()}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
