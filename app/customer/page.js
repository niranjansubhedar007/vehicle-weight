...
const Customer = () => {
  ...
  const handleDeleteConfirmation = async () => {
    try {
      ...
      const response = await axios.delete(
        `http://16.170.231.171/api/coustomer/coustomers/${deleteItemId}`
      );
  ...
  useEffect(() => {
    if (editItemId) {
      ...
      axios
        .get(`http://16.170.231.171/api/coustomer/coustomers/${editItemId}`)
        .then((response) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Make a POST request to your backend API
      const response = await axios.post(
        "http://16.170.231.171/api/coustomer/coustomers",
        formData
      );
      console.log("Customer added:", response.data);
      // Reset the form data
  ...
  const handleEditSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `http://16.170.231.171/api/coustomer/coustomers/${editItemId}`,
        formData
      );
