...
const Weight = () => {
    ...
    const fetchDataAndRSTNo = async () => {
      try {
        const [vehicleResponse, rstNoResponse] = await Promise.all([
          ...
          axios.get("http://16.170.231.171/api/weight/weight/getNextRSTNo"),
        ]);
  ...
  //     await axios.post("http://16.170.231.171/api/weight/weights", weightData);
  ...
  const closePopup = () => {
    ...
    const handleEditSubmit = async (e) => {
      ...
      try {
        ...
        await axios.patch(`http://16.170.231.171/api/weight/weights/${editingWeight._id}`, weightData);
