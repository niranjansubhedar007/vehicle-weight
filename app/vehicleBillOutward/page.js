    ...
    const fetchDataAndRSTNo = async () => {
      try {
        const [vehicleResponse, rstNoResponse] = await Promise.all([
          ...
          axios.get("http://localhost:5000/api/weightOutward/weight/getNextRSTNo"),
        ]);
...
//   };
  ...
  useEffect(() => {
    const fetchWeights = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/weightOutward/weights");
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      await axios.patch(`http://localhost:5000/api/weightOutward/weights/${editingWeight._id}`, weightData);
